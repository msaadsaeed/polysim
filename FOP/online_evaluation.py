...
def test(model, face_test, voice_test, test_label):

    model.eval()
    model.cuda()
 
    # if args.cuda:
    face_test, voice_test, test_label = face_test.cuda(), voice_test.cuda(), test_label.cuda()

    face_test, voice_test = Variable(face_test), Variable(voice_test)
    with torch.no_grad():
        comb, face, voice = model(face_test, voice_test)
        
        

        
        # logits = comb[1].detach().cpu().numpy()
        
        logits = comb[1]
        preds = torch.argmax(logits, dim=1)
        # preds = preds.detach().cpu().numpy()
        correct = (preds == test_label).sum().item()
        acc = 100*correct / test_label.size(0)
        
        test_label = test_label.detach().cpu().numpy()
        preds = preds.detach().cpu().numpy()
        
        f1 = metrics.f1_score(test_label, preds, average="macro")
