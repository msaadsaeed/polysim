...
def load_ds(csv_path, mod, encoder=None):
    afeat, ffeat, labels = [], [],  []
    ...
    for aud, face, label in tqdm(ds, leave=False):
        afeat.append(aud)
        ffeat.append(face)
        labels.append(label)

    return np.stack(afeat), np.stack(ffeat), np.array(labels)

def read_data(csv):
    
    encoder = "ecappa_feats_path"

    # train_csv = f"./feature_tracker/{ver}_train_{seen_lang}.csv"
    # val_csv   = f"./feature_tracker/{ver}_val_{seen_lang}.csv"
    # test_csv  = f"./feature_tracker/{ver}_test_{seen_lang}.csv"
    # unseen_val_csv  = f"./feature_tracker/{ver}_val_{unseen_lang}.csv"
    # unseen_test_csv = f"./feature_tracker/{ver}_test_{unseen_lang}.csv"

    face_train, voice_train, train_label = load_ds(csv, mod="audiovisual", 
                                                   encoder=encoder)
    # face_test, voice_test, test_label = load_ds(test_csv, "audiovisual", encoder)
    # face_val, voice_val, val_label = load_ds(val_csv, "audiovisual", encoder)
    ...
    print("Train file length", len(face_train))
        
    print('Shuffling\n')
    combined = list(zip(face_train, voice_train, train_label))
    img_train = []
    voice_train = []
    train_label = []
...
ver = "v1"
seen_lang = "English"
unseen_lang = "Urdu" if seen_lang=="English" else "English"

print(
    f"\nVersion={ver} | Seen={seen_lang} | Unseen={unseen_lang}"
)

face_train, voice_train, train_label = read_data(f"../feature_tracker/{ver}_train_{seen_lang}.csv")
face_test, voice_test, test_lable = read_data(f"../feature_tracker/{ver}_test_{seen_lang}.csv")
